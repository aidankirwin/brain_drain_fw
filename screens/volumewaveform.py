...
class VolumeWaveform(LayoutDesigns):
    ...
    def create_numpad(self):
        ...
        def numpad_click(label):
            ...
            if label == '⌫':
                ...
                if self.target_volume_value is not None:
                    self.controller.update_target_volume(self.target_volume_value)
    ...
    def update_display_text(self):
        ...
        # Grab ICP value
        try:
            self.target_volume_value = int(val) if val else None
        except ValueError:
            self.target_volume_value = None
        ...
        if ranges:
            ...
            if "mL" in content:
                # Find the index of "mL"
                idx = widget.search("mL", "1.0")
    ...
    def setup_ui(self):
        self.configure(bg="#f3eaf9")

        outer_frame = tk.Frame(self, highlightbackground="#8e44ad", highlightthickness=3, bg="white")
        ...
        # --- DRAINAGE FLOW RATE ---
        self.target_volume = tk.Text(grid_container, bg="white", fg="black", height=8, width=20, 
                                  borderwidth=0, padx=10, pady=5, highlightthickness=0)
        self.target_volume.grid(row=0, column=1, rowspan=2, sticky="nsew", padx=(0,1), pady=(1,0))
        
        self.target_volume.tag_configure('big_font', font=('Helvetica', 70), justify='center')
        self.target_volume.tag_configure('normal_font', font=('Helvetica', 20), justify='center')
        self.target_volume.tag_configure('small_font', font=('Helvetica', 16), justify='right')
        
        self.target_volume.tag_configure("val", font=('Helvetica', 70))
        # This setting helps the tag 'grab' new characters typed at its edges
        self.target_volume.tag_raise("val")

        self.target_volume.insert(tk.END, "\n", "small_font")
    
        self.target_volume.insert(tk.END, "Target Volume: \n", "normal_font") 
        self.target_volume.insert(tk.END, "12", ("big_font", "val")) 
        self.target_volume.insert(tk.END, " mL/hr", "normal_font")

        self.target_volume.image_create("end", image = target_icon)
        self.images["target_icon"] = target_icon  # Keep reference to avoid garbage collection

        self.target_volume.bind("<Button-1>", lambda e: self.show_numpad(e, self.target_volume))
    ...
    def update_current_volume(self):
        ...
        self.waveform.after(10, self.update_current_volume)
